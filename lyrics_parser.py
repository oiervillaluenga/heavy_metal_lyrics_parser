...
if os.path.isfile(all_data_file) == False:
    #If there isn't we proceed to the data extraction
    try:
        list_years  = [fp.create_year_range(x) for x in range(1970,2020)]
        name_years  = [x for x in range(1970,2020)]
    except Exception as e: 
    ...
    for year,name in zip(list_years,name_years):
